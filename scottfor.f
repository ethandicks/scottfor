...
C
       ...
       CHARACTER(80) ROOM_NAME, RN1, RN2
...
C   CHECK FOR SYNONYMS (STARTS WITH '*')
         IF (IS_SYN .EQ. 1) THEN
           WORD_FIRST = WORD_FIRST - 1
         ENDIF
C   COPY THE TRIMMED WORD
         WORD = TRIM(RAW_WORD(WORD_FIRST:WORD_LAST))

         IF (PRINT_WORDS .NE. 0) THEN
...
407    FORMAT (6I3, A)
       DO 410 I=1, RL+1
         DO 420 J=1, 6
           READ (2, 400) RM(I, J)
420      CONTINUE
         READ (2, 405) ROOM_NAME

C   CHECK FOR ADDITIONAL LINES IN THE ROOM DESCRIPTION
         STRLEN = LEN(ROOM_NAME)
         DO WHILE (ROOM_NAME(STRLEN:STRLEN) .EQ. ' ')
           STRLEN = STRLEN - 1
         ENDDO
         IF (ROOM_NAME(STRLEN:STRLEN) .NE. '"') THEN
           PRINT 406, 'ROOM #', I, ' HAS A SECOND LINE'
           READ (2, 405) RN2
           STRLEN = LEN(RN2)
           DO WHILE (RN2(STRLEN:STRLEN) .EQ. ' ')
             STRLEN = STRLEN - 1
           ENDDO
           IF (RN2(STRLEN:STRLEN) .NE. '"') THEN
             PRINT 406, 'ROOM #', I, ' HAS A THIRD LINE'
             READ (2, 405) RN3
           ENDIF
         ENDIF
...
C    PRINT ROOM DESCRIPTIONS IF ENABLED
         IF (PRINT_ROOMS .NE. 0) THEN
           PRINT 407,I,RM(I,1),RM(I,2),RM(I,3),RM(I,4),RM(I,5),ROOM_NAME
         ENDIF
410    CONTINUE
